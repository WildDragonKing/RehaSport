...
export default function TrainersPage(): JSX.Element {
    ...
    return (
      <div className="text-center py-12">
        <p className="text-sage-600 dark:text-sage-300">
          Nur Administratoren können Trainer verwalten.
        </p>
      </div>
    );
  }
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      setSuccess(
        `Einladung für ${email} erstellt. Der Trainer kann sich jetzt unter /login mit Google anmelden.`,
      );
      setEmail("");
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-sage-500 dark:text-sage-400">
                    Eingeladen am{" "}
                    {inv.createdAt?.toDate?.().toLocaleDateString("de") ||
                      "Unbekannt"}
                    {inv.expiresAt && (
                      <>
                        {" "}
                        • Gültig bis{" "}
                        {inv.expiresAt.toDate().toLocaleDateString("de")}
                      </>
                    )}
                  </p>
                </div>
