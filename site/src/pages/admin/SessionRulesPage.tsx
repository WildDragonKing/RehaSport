...
export default function SessionRulesPage(): JSX.Element {
  ...
  const updatePhase = (index: number, field: keyof SessionPhaseConfig, value: string | number) => {
  ...
  const totalPhaseDuration = config.phases.reduce((sum, p) => sum + p.duration, 0);

  if (!isAdmin) {
    return (
      <div className="text-center py-12">
        <p className="text-sage-600">Nur Administratoren können die Stunden-Regeln verwalten.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-sage-900">Stunden-Regeln</h1>
          <p className="mt-2 text-sage-600">Konfiguriere das Schema für Trainingsstunden</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-sage-200 p-6">
        <h2 className="text-lg font-semibold text-sage-900 mb-4">Gesamtdauer</h2>
        <div className="flex items-center gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => setConfig({ ...config, totalDuration: parseInt(e.target.value) || 45 })}
              min={15}
              max={120}
              className="w-24 px-3 py-2 border border-sage-300 rounded-lg focus:ring-2 focus:ring-sage-500"
            />
          </div>
          <div className={`px-4 py-2 rounded-lg ${totalPhaseDuration === config.totalDuration ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`}>
            <span className="font-medium">Phasen-Summe: {totalPhaseDuration} Min</span>
            {totalPhaseDuration !== config.totalDuration && (
              <span className="ml-2 text-sm">
                ({totalPhaseDuration > config.totalDuration ? '+' : ''}{totalPhaseDuration - config.totalDuration} Min)
              </span>
            )}
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-sage-200 p-6">
        ...
        <div className="space-y-4">
          {config.phases.map((phase, index) => (
            <div key={phase.id} className="p-4 bg-sage-50 rounded-lg border border-sage-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <label className="block text-xs text-sage-600 mb-1">Dauer (Min)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs text-sage-600 mb-1">Min. Übungen</label>
                  <input
                    type="number"
                    value={phase.minExercises}
                    onChange={(e) => updatePhase(index, 'minExercises', parseInt(e.target.value) || 1)}
                    min={1}
                    max={20}
                    className="w-full px-2 py-1.5 text-sm border border-sage-300 rounded focus:ring-1 focus:ring-sage-500"
                  />
                ...
                <div>
                  <label className="block text-xs text-sage-600 mb-1">Max. Übungen</label>
                  <input
                    type="number"
                    value={phase.maxExercises}
                    onChange={(e) => updatePhase(index, 'maxExercises', parseInt(e.target.value) || 1)}
                    min={1}
                    max={20}
                    className="w-full px-2 py-1.5 text-sm border border-sage-300 rounded focus:ring-1 focus:ring-sage-500"
                  />
                ...
                <div className="md:col-span-1">
                  <label className="block text-xs text-sage-600 mb-1">Beschreibung</label>
                  <input
                    type="text"
      ...
      {/* Guidelines */}
      <div className="bg-white rounded-xl shadow-sm border border-sage-200 p-6">
        <h2 className="text-lg font-semibold text-sage-900 mb-4">Richtlinien</h2>
      ...
      {/* Contraindications */}
      <div className="bg-white rounded-xl shadow-sm border border-sage-200 p-6">
        <h2 className="text-lg font-semibold text-sage-900 mb-4">Allgemeine Kontraindikationen</h2>
        <p className="text-sm text-sage-600 mb-3">Eine Kontraindikation pro Zeile</p>
        <textarea
      ...
      <div className="bg-sage-50 rounded-xl border border-sage-200 p-6">
        <h2 className="text-lg font-semibold text-sage-900 mb-4">Vorschau: Stunden-Schema</h2>
        <div className="flex gap-1 h-8 rounded-lg overflow-hidden">
          {config.phases.map((phase, index) => {
            const percentage = (phase.duration / config.totalDuration) * 100;
            const colors = ['bg-sage-300', 'bg-sage-400', 'bg-sage-500', 'bg-sage-600', 'bg-sage-700'];
            return (
              <div
                key={phase.id}
        ...
        <div className="flex gap-1 mt-2">
          {config.phases.map((phase, index) => {
            const percentage = (phase.duration / config.totalDuration) * 100;
            return (
              <div key={phase.id} style={{ width: `${percentage}%` }} className="text-xs text-sage-600 text-center truncate">
                {phase.name}
              </div>
            );
          })}
